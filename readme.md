...
![tictactoe_model_multidimension.drawio_2.png](./readme/tictactoe_model_multidimension.drawio (3).png) 

브라우저에서 돌리는 것 자체가 불가하다는 결론. 시작하기 전에 이미 알고 있었고 직접 계산해보니 아득하지만
그렇다고 안해볼 것은아니였고 어찌 적당한 결론을 내야겠지.

덜어냄의 미학이냐 최적화냐 다른 방법이냐..

따라서 아래 방법을 정리해본다.

1. `차원 갯수 조절`: 차원의 갯수가 복수개의 홀수인 경우 특정 플레이어의 다음 수를 찾을 수 있음. 이를 포기하고 단일 차원에서 또는 더 빠른 차원에서 계산함. (지금까지 보면 2개 차원 계산 시 3~5초 경과함) 1개, 3개 차원으로 한다면?
```markdown
지능이 길 가던 누렁이 같음.
봐주는 체스 그랜드 마스터 정도의 난이도를 원함.
```

2. `ai모델, 로직 기반 섞음`:  확실히 지능 및 성능이 올라가겠으나..
```markdown
목적에 부합하지 않음.
```

3. `모델 성능 강화`: 연산하는 차원 수가 적어도 좋은 지능이 나오고 각 확률 생성 시 걸리는 시간을 최소화 하여 시간 단축을 노림.
```markdown
확실히 성능이 덜떨어지는 이유는 모델의 샘플이 최소 4~5천만개 필요한데 지금 고작 10만개도 안됨.
모델 튜닝 및 앙상블을 통한 단일화를 목표로 했음. 앙상블은 아직 능지 부족으로 어렵고.. 데이터는 고품질이 좋겠으나.. 어디 있을리가 없음.
```
